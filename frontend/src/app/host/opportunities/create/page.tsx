...
export default function CreateOpportunityPage() {
  ...
  const { addOpportunity, userProfile } = useApp();
  ...
  const [eligibility, setEligibility] = useState('Open to B.Tech/BCA/MCA students');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      addOpportunity({
        ...
        description,
        category,
        host: userProfile?.name || 'NCR Tech Labs',
        date,
        ...
        price,
        mode,
        registrationUrl: registrationUrl || `/register-event/opp-host-${Date.now()}`,
        source: 'HOST',
      ...
      setError('Failed to publish opportunity. Check form fields and try again.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      ...
      {/* Title */}
      <div>
        <h1 className="text-2xl font-black text-slate-950 tracking-tight leading-none">Create Opportunity</h1>
        <p className="text-xs text-slate-400 mt-1.5 font-bold uppercase tracking-wider">Publish a new event to the student network</p>
      </div>
